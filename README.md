...
在线演示：[GitHub Pages](https://zaowenchen.github.io/planar-2r-simulator/)

技术栈：React、TypeScript、Three.js / React Three Fiber、Zustand、KaTeX 与 Plotly。
...
- **运动学**：以度和毫米联动关节角与末端位姿；15 步教学链由 6 步 D–H 正运动学、5 步解析几何位置逆运动学（含正运动学回代）和 4 步几何雅可比组成，并用奇异值、条件数、Yoshikawa 可操作度与逆条件数解释位置灵巧度。三维场景同步展示世界/连杆/末端坐标系、D–H 四段变换、工作平面、肘上/肘下幽灵构型、回代误差和姿态差异，二维图负责解释三角形关系。
- **动力学（未完成）**：当前可编辑质量、质心、惯量、重力与粘性摩擦，并查看惯性矩阵、科氏项、重力项、力矩、能量和功率；该模块仍处于开发阶段，结果仅用于教学展示。
- **轨迹示教**：像使用机器人示教器一样通过关节 JOG 记录 P1、P2 等点位，创建关节空间 PTP 指令，用五次多项式或梯形速度生成轨迹；大尺寸 3D、数学公式、归一化多项式与 $q/\dot q/\ddot q$ 曲线共享时间轴，可导出 CSV。
...
- 运动学、机器人模型与轨迹示教界面统一使用毫米和度；三角函数、动力学和数据导出的核心值仍使用米、弧度及其他 SI 单位。
...
- 轨迹示教只生成运动学轨迹，不计算惯性、力矩、能量或动力学响应；已有动力学分析模块保持独立。
...
- PTP 示教界面使用度、度每秒和度每二次方秒，轨迹核心与 CSV 仍保持弧度制；五次多项式峰值采用解析计算，曲线使用 201 个等间隔样本。
