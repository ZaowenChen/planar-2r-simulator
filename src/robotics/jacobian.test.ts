...
describe('geometric Jacobian', () => {
  ...
  it('detects a straight-arm position singularity with an infinite condition number', () => {
    ...
    expect(metrics.inverseConditionNumber).toBe(0)
    expect(metrics.yoshikawaManipulability).toBeLessThanOrEqual(1e-12)
  ...
  it('reports a nonsingular position Jacobian above the configured threshold', () => {
    ...
    expect(metrics.inverseConditionNumber).toBeCloseTo(1 / metrics.conditionNumber, 12)
    expect(metrics.inverseConditionNumber).toBeGreaterThan(0)
    expect(metrics.inverseConditionNumber).toBeLessThanOrEqual(1)
    expect(metrics.yoshikawaManipulability).toBeGreaterThan(0)
