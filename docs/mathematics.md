...
轨迹示教页面只使用上述 $q,\dot q,\ddot q$ 驱动 3D 姿态、公式代入和曲线，不把轨迹代入动力学方程，也不显示力矩、能量或功率。正弦轨迹函数保留为数学核心能力，但第一版 PTP 示教器只开放五次多项式和梯形速度。
...
动力学核心仍保留常值、阶跃、正弦和分段常值力矩，以及固定步长 RK4 积分器。积分器会在力矩不连续点拆分步长，并确保样本包含精确终止时刻；若出现非有限数值、病态惯性矩阵或越过关节限位，则给出诊断并停止。轨迹示教页面不调用这条正动力学计算链。
